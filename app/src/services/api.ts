...
export class ApiClient {
  ...
  async pairInit(): Promise<{ expiresInMs: number }> {
    return this.typedRequest<{ expiresInMs: number }>("/api/auth/pair/init", {
      method: "POST",
    });
  }
  ...
  async listSessions(accessToken: string): Promise<SessionInfo[]> {
    const resp = await this.typedRequest<{ sessions: SessionInfo[] }>(
      "/api/sessions",
      { accessToken },
    );
  ...
  async createSession(
    accessToken: string,
    title?: string,
  ): Promise<Pick<SessionInfo, "id" | "title">> {
    return this.typedRequest<Pick<SessionInfo, "id" | "title">>(
      "/api/sessions",
      {
        method: "POST",
  ...
  async listModels(
    accessToken: string,
  ): Promise<{ models: string[]; current: string }> {
    return this.typedRequest<{ models: string[]; current: string }>(
      "/api/models",
      { accessToken },
    );
  }
  ...
  async switchModel(accessToken: string, model: string): Promise<void> {
    await this.request("/api/models/current", {
      method: "PUT",
      body: { model },
      accessToken,
    });
  ...
  async getStatus(): Promise<{ status: string; version: string }> {
    return this.typedRequest<{ status: string; version: string }>(
      "/api/status",
    );
  }
  ...
  async listProviders(
    accessToken: string,
  ): Promise<{ providers: ProviderInfo[]; activeProviderId: string }> {
    return this.typedRequest<{
      providers: ProviderInfo[];
      activeProviderId: string;
    }>("/api/providers", { accessToken });
  }
  ...
  async verifyProvider(
    ...
    return this.typedRequest<{ ok: boolean; error?: string }>(
  ...
  async fetchProviderModels(
    ...
    const resp = await this.typedRequest<{ models: string[] }>(
