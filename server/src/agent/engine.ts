  ...
  async function runLlmLoop(
    ...
    for (let round = 0; round < config.maxToolRounds; round++) {
      ...
      if (response.type === "text") {
        return handleTextResponse(response.content, sessionId, round, startedAt);
      }

      await handleToolCalls(response.toolCalls, sessionId, chatId, round, messages);
    }

    return buildOverflowResponse(sessionId, startedAt);
  }
