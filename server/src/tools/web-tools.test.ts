...
describe("webFetchTool", () => {
  ...
  it("should throw on fetch failure", async () => {
    ...
    await expect(webFetchTool.execute({ url: "https://example.com" })).rejects.toThrow(
      "Failed to fetch",
    );
  });

  it("should throw on invalid URL", async () => {
    await expect(webFetchTool.execute({ url: "ftp://example.com" })).rejects.toThrow();
  });

  it("should throw on private network URL", async () => {
    await expect(webFetchTool.execute({ url: "http://localhost:3000" })).rejects.toThrow(
...
describe("webSearchTool", () => {
  ...
  it("should throw on HTTP error", async () => {
    ...
    await expect(webSearchTool.execute({ query: "test" })).rejects.toThrow("Search failed");
  });
});
