...
describe("createStreamingTokenHandler", () => {
  it("should send token message via websocket", () => {
    ...
    expect(messages[0]).toEqual({ type: "token", sessionId: "sess-1", content: "Hello" });
    expect(messages[1]).toEqual({ type: "token", sessionId: "sess-1", content: " world" });
  });
...
describe("sendFinalResponse", () => {
  it("should send response message via websocket", () => {
    ...
    expect(msg).toEqual({ type: "response", sessionId: "sess-1", content: "Final answer" });
  });
...
describe("sendError", () => {
  it("should send error message with code and message", () => {
    ...
    const msg = JSON.parse(getSent()[0]!) as Record<string, unknown>;
    expect(msg).toEqual({ type: "error", sessionId: "sess-1", code: "TIMEOUT", message: "Request timed out" });
  });
  ...
  it("should send error message without sessionId", () => {
    ...
    const msg = JSON.parse(getSent()[0]!) as Record<string, unknown>;
    expect(msg).toEqual({ type: "error", sessionId: undefined, code: "INVALID_MESSAGE", message: "Bad format" });
  });
